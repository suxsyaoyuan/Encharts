...
(function () {
  ...
  // 4. tab切换效果制作
  // 准备不同标签的数据 绑定点击事件 激活样式 切换数据 开启定时器
  // (2) 点击切换效果
  $(".sales .caption").on("click", "a", function () {
    // 点击时要注意这个索引号的问题 把当前索引号给index 不然会乱
    index = $(this).index() - 1;
    // 点击当前a 高亮显示 当前a调用active 其他a去掉active
    $(this).addClass("active").siblings("a").removeClass("active");
    // 在数据中用h5的自定义属性 拿到当前a 的自定义属性值
    ...
    // console.log(arr); 返回的二维数组 里面有两个数组
    // 根据拿到的数据重新渲染 series里面的data值 series里面两个对象
...
(function () {
  ...
  // (1) 遍历 hotData对象 五个城市的数据
  var supHTML = "";
  $.each(hotData, function (index, item) {
    // console.log(item); item/城市名字/销售额
    supHTML += `<li><span>${item.city}</span><span> ${item.sales} <s class=${
      item.flag ? "icon-up" : "icon-down"
    }></s></span></li>`;
  });
  // 把生成的5个小li字符串给 sub dom盒子 转换为标签
  $(".sup").html(supHTML);

  // 3. 当鼠标进入 tab 的时候
  // 鼠标经过当前的小li要高亮显示 鼠标经过要把当前li的index重置给index
